...
[`ghcr.io/ferretdb/postgres-documentdb:17-0.102.0-ferretdb-2.1.0`](https://ghcr.io/ferretdb/postgres-documentdb:17-0.102.0-ferretdb-2.1.0)
...
2. If you have `psql` installed, you can connect to the PostgreSQL with DocumentDB extension with the following command:
   ...
   ```sh
   docker run --rm -it ghcr.io/ferretdb/postgres-documentdb:17-0.102.0-ferretdb-2.1.0 \
     psql postgres://<username>:<password>@localhost:5432/postgres
   ```
