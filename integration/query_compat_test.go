...
func TestDoubleNearMaxPrecision(t *testing.T) {
	...
	maxDouble := float64(2<<53) - 10000000

	var insertDocs []any

	for i := 0; i < 100; i++ {
		val := maxDouble - float64(i)
		insertDocs = append(insertDocs, bson.D{{"_id", i}, {"v", val}})
	}
	...
	targetCursor, targetErr := targetCollection.Find(ctx, bson.D{})
	compatCursor, compatErr := compatCollection.Find(ctx, bson.D{})
