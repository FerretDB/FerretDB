...
## Comparison of BSON
The comparison of BSON is about finding if the BSON value is greater or
less than the other BSON value.
Comparison is used in sorting, filtering with `$lt`, `$gt`, `lte` and `$gte` operators,
and updating with `$max` and `$min` operators.
Comparing the same BSON type is straight forward, comparing different BSON type
is defined in [BSON Comparison Order](#bson-comparison-order).
However, array comparison with other BSON type may use BSON comparison order,
...
BSON values have comparison order from the lowest to the highest as it's listed in the following.
...
For the different BSON types BSON comparison order is used,
for the same BSON type the comparison specific to that type is used.

### Comparing Arrays

To compare BSON arrays, each index of the arrays are iterated in order to compare
the BSON comparison order, then the values.
As soon as they are different, that is used for the result of the comparison.

During iterating arrays, if no more element exists in the next index,
then that array is smaller. Because of this empty array is less than arrays with elements.

#### Example of Comparing Arrays
Two arrays compare each element of the array, until they are not the same.
For example comparing `[3, 1, 'b']` and `[3, 'b', 'a']` take following steps.
1. Compare `[3, 1, 'b']` and `[3, 2, 'a']` using comparison order. They are both arrays.
...
4. Compare next index, `1` and `b` using comparison order, `1` is number and 
   less than string `b`.
So `[3, 1, 'b']` is less than `[3, 'b', 'a']`.

### Comparing Array and Other BSON Type
Comparing array with other BSON types doesn't _always_ use
comparison order of array BSON type. It may use the largest or the smallest element
from the array to compare against non-array BSON type.
...
### Comparing Array Within Array - Nested Array Comparison
...
Descending sort uses the largest element of the array for comparison.
