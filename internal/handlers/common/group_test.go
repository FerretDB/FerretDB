...
func TestCountSumAndAverage(t *testing.T) {
	...
	assert.Equal(t, "SELECT TO_CHAR(TO_TIMESTAMP((_jsonb->'date'->>'$d')::numeric / 1000), 'YYYY-MM-DD') AS _id, SUM((_jsonb->'quantity')::numeric) AS totalSaleAmount, AVG((_jsonb->'quantity')::numeric) AS averageQuantity, SUM(1) AS count FROM %s GROUP BY _id", ctx.GetSubQuery())
}
