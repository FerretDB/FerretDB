...
[Sorting](#comparison-in-sorting), used in `.sort({<<field>>: <<order>>})`, uses the following rules
...
[Filtering](#comparison-in-filter-operator), commonly used in `$lt` and `$gt` operators, applies the following rules to compare array and other BSON types.
...
the value that exists in the document. In this example `{$max: {v: 'c'}}` is used
...
Since then, we took time to understand the under the hood of comparison for different operations.

Finally, we found out that nested array has unexpected behaviour on 
[`$gt` operator](#filter-operator-with-nested-array). A typical assumption
on how `$gt` may work with nested array would be similar to how `$lt` works
with nested array but using the largest element. This was not the case, and
we are yet to find out how it should function.
Perhaps using `$gt` in nested array is not a common usage and the result
we found was an edge case.
