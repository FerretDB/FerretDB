...
type command struct {
	...
	// The doc is decoded for the top level fields.
	Handler func(ctx context.Context, msg *wire.OpMsg, doc *wirebson.Document) (*wire.OpMsg, error)
...
func (h *Handler) initCommands() {
	...
	for name, cmd := range h.commands {
		...
		h.commands[name].Handler = func(ctx context.Context, msg *wire.OpMsg, doc *wirebson.Document) (*wire.OpMsg, error) {
			if err := checkAuthentication(ctx, name, h.L); err != nil {
				return nil, err
			}

			return cmdHandler(ctx, msg, doc)
		}
	}
}
