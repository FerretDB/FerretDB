...
The `createIndexes` command creates one or more indexes on a given collection to improve query performance
or enable certain query features (e.g., full-text, vector, and TTL indexes).
...
{/* Use $type aliases from https://pkg.go.dev/github.com/FerretDB/wire@v0.1.1/wirebson#hdr-Types */}
{/* in the Type column, or `any`, if any type is allowed (for example, for the `comment` field). */}
{/* Use `int, long, or object` if those three types are allowed. */}
{/* Do not use `document`, `number`, `integer`, `int32` or any other form. */}
...
| `key`                     | object | A single document that contains the field(s) to index and their sort order or type (e.g. `1` for ascending, `-1` for descending, `text` for full-text indexes, `cosmosSearch` for vector indexes, and `2dsphere` for geospatial indexes). |
| `name`                    | string | A required name for the index. Must be unique to the collection.                                                                                                                                                                          |
| `unique`                  | bool   | Optional. Ensures all values are unique for the indexed field if set to `true`.                                                                                                                                                           |
| `partialFilterExpression` | object | Optional. Restricts index to documents matching the condition.                                                                                                                                                                            |
| `sparse`                  | bool   | Optional. Only indexes documents that contain the indexed field if set to `true`.                                                                                                                                                         |
| `expireAfterSeconds`      | int    | Optional. Specifically used for creating TTL indexes. Automatically deletes documents after set time.                                                                                                                                     |
| `cosmosSearchOptions`     | object | Optional. Specifies the configuration parameters (`kind`, `similarity`, `dimensions`, etc.) for creating a vendor index. [See vector index guide for more details](../../guides/vector-search.mdx).                                       |
