...
func TestCommandsAdministrationDBStatsEmpty(t *testing.T) {
	...
	assert.Equal(t, int32(2), must.NotFail(doc.Get("collections")))
	...
	assert.Equal(t, float64(8192), must.NotFail(doc.Get("dataSize")))
	...
	assert.InDelta(t, float64(24576), must.NotFail(doc.Get("totalSize")), 8192)
...
func TestCommandsAdministrationDBStatsWithScale(t *testing.T) {
	...
	assert.InDelta(t, float64(16.384), must.NotFail(doc.Get("dataSize")), 10)
	assert.InDelta(t, float64(1), must.NotFail(doc.Get("indexes")), 1)
