...
FROM --platform=$BUILDPLATFORM golang:1.24.4 AS eval-dev-prepare
...
FROM golang:1.24.4 AS eval-dev-build
...
# FROM ghcr.io/ferretdb/postgres-documentdb-dev:17-0.104.0-ferretdb-2.3.0 AS eval-dev

# Use moving development image during development.
FROM ghcr.io/ferretdb/postgres-documentdb-dev:17-pr-prepare-release AS eval-dev
