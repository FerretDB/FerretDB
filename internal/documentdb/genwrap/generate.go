...
// param represents name and type of Go parameter.
type param struct {
	Name string
	Type string
}
...
func Generate(writer io.Writer, data *templateData) error {
	q, sqlArgs := generateSQL(data)
	data.QueryRowArgs = append([]string{"ctx", fmt.Sprintf(`"%s"`, q)}, sqlArgs...)
	...
	data.Params = append(params, data.Params...)

	scanArgs := make([]string, len(data.Returns))
	...
	data.Returns = append(data.Returns, param{
		Name: "err",
		Type: "error",
	})
...
func generateSQL(f *templateData) (string, []string) {
	...
	if f.IsProcedure {
		q := fmt.Sprintf("CALL %s(%s)", f.SQLFuncName, f.SQLArgs)
		return q, args
	}
