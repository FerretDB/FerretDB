...
[Sorting](#sorting), used in `.sort({<<field>>: <<order>>})`, uses the following rules
...
[Filtering](#gt-lt-filter-operator), commonly used in `$lt` and `$gt` operators, applies the following rules to compare array and other BSON types.
...
[Updating](#max-min-update-operator) with `$max` and `$min` operators applies the following rules
to compare array and other BSON types.
