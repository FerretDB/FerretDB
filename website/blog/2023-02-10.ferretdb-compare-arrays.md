...
When comparing two BSON arrays, each index is iterated in order.
At each index the BSON comparison order then the value comparison
is used.
If the values at a particular index are not equal, that result is used as the final comparison result.

If there are no more elements in the next index of either array during the iteration,
...
For instance, when comparing the arrays `[3, 1, 'b']` and `[3, 'b', 'a']`, the following steps are taken:

1. Compare `[3, 1, 'b']` and `[3, 2, 'a']` using comparison order.
...
4. Compare next index, `1` and `b` using comparison order, `1` is a number and less than string `b`.
...
* For array and other BSON type comparison, ascending sort uses the smallest element of any BSON type from the array.
* For array and other BSON type comparison, descending sort uses the largest element of any BSON type from the array.
* empty array is less than null.

[Filtering](#gt-lt-filter-operator) used in `$lt` and `$gt` operators use following rules
to compare array and other BSON types.
* For array and other BSON type comparison, `$lt` uses the smallest element of the same BSON type as other BSON type from the array.
* For array and other BSON type comparison, `$gt` uses the largest element of the same BSON type as other BSON type from the array.
...
Comparison of two nested arrays is similar to array comparison.
...
3. Check comparison order of the first index within the nested array `1` and `a`, `1` is number and `a` is string.
The number is smaller in comparison order.
...
`array-mixed` record is smaller than
...
The descending sort uses the largest element of the array.
...
Like [arrays](#sorting-arrays) nested array gets the largest or smallest element
from an array which is another array. Hence array is used to compare using
BSON comparison order.
For comparing with other nested arrays, each index is iterated to use comparison order
...
4. Compare the first indexes of the array using BSON comparison order, `null` is null type and `6` is number. 
5. Compare the second index of the array, first nested array has `1` and second has `2`
...
`$lt` selects records that are less than a specific value but this is only applied to the same BSON type. 
...
```
The records selected are string BSON type `string` where `b` is less than `c`,
and also array BSON type `array-mixed` which containing string BSON type `a` and `z` in its array.
`$lt` uses the smallest value `a` where `a` is less than `b`.

It does not match `array-string` because the smallest element of the array `d` is not less than `c`.
It also does not match `null` because is it not string BSON type.
It does not match `array-empty` and `array-number` because the array does not contain string BSON type.
...
[
  ...
  { _id: 'array-string', v: [ 'd' ] }         // `d` the largest string is compared to `c`
]
```

The records selected are `array-mixed` which contains `z` which is greater than `c`,
and `array-mixed` which contains `d`.

It does not match `string` because `b` is not greater than `c`.
It also does not match `null` because is it not string BSON type.
It does not match `array-empty` and `array-number` because the array does not contain string BSON type.
#### Filtering arrays
...
* `array-mixed` is selected by comparing
  each element and first element `0` is less than `c` in comparison order.
* `array-number` is selected by comparing
  each element and first element `10` is less than `c` in comparison order.
* `array-string` is not selected because comparing
...
* `array-null` is selected because first index of nested array `null`
  less than string `c` in BSON comparison order.
...
It's interesting that `nested-number` and `array-null` were selected because their first index
contain non string type. It appears that `$lt` filter compares different BSON type as long as
they are within an array, instead of filtering it out.

##### $gt

Using array filter `['c']` on `$gt` operator, it selected all nested arrays
from the collection.
This is unexpected since empty array
