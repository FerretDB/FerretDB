...
func TestCreateIndexesCommandCompat(t *testing.T) {
	...
	for name, tc := range map[string]struct { //nolint:vet // for readability
		...
		skipForPg string // optional, skip test for the old PG backend with a specified reason
	}{
		...
		"ExistingNameDifferentKeyLength": {
			...
			skipForPg:      "https://github.com/FerretDB/FerretDB/issues/3435", // old PG handler doesn't validate this case correctly
		},
		"InvalidKey": {
			collectionName: "test",
			key:            42,
		...
		t.Run(name, func(t *testing.T) {
			...
			if tc.skipForPg != "" {
				setup.SkipForPostgreSQL(t, tc.skipForPg)
			}
