...
func (h *Handler) MsgDelete(ctx context.Context, msg *wire.OpMsg) (*wire.OpMsg, error) {
	...
	for i := 0; i < deletes.Len(); i++ {
		...
		del, err := h.processDelete(ctx, sp, filter, limit)
		if err == nil {
			deleted += del
			continue
		}
...
func (h *Handler) processDelete(ctx context.Context, sp *pgdb.SQLParam, filter *types.Document, limit int64) (int32, error) {
	var err error
