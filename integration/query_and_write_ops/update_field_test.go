...
func TestUpdateFieldSetUpdateManyUpsert(t *testing.T) {
	...
	for name, tc := range map[string]struct { //nolint:vet // used for testing only
		...
		t.Run(name, func(t *testing.T) {
			...
			integration.AssertEqualDocumentsSlice(t, []bson.D{expected}, res)
		})
	}
}
...
func TestUpdateCommandUpsert(t *testing.T) {
	...
	for name, tc := range map[string]struct { //nolint:vet // used for testing only
		...
		t.Run(name, func(t *testing.T) {
			...
			if tc.err != nil {
				assert.Nil(t, res)
				integration.AssertEqualWriteError(t, *tc.err, err)
			...
			for i, elem := range tc.findRes {
				doc := integration.ConvertDocument(t, elem)

				actualDoc := integration.ConvertDocument(t, findRes[i])
