...
func collectionsStats(ctx context.Context, p *pgxpool.Pool, dbName string, list []*metadata.Collection, refresh bool) (*stats, error) { //nolint:lll // for readability
	...
	if refresh {
		...
		// However, the purpose is to update statistics of tables in `pg_class`
		// from recent delete operations instead of reclaiming space.
	...
	// however it is not updated immediately after operation such as DELETE
