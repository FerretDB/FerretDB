...
func TestPlain(t *testing.T) {
	...
	t.Run("Exported", func(t *testing.T) {
		doc1, err := PlainHash(WrapPassword("password"))
		require.NoError(t, err)

		doc2, err := PlainHash(WrapPassword("password"))
		require.NoError(t, err)

		testutil.AssertNotEqual(t, doc1, doc2)

		err = PlainVerify(WrapPassword("password"), doc1)
		assert.NoError(t, err)

		err = PlainVerify(WrapPassword("password"), doc2)
		assert.NoError(t, err)

		err = PlainVerify(WrapPassword("wrong"), doc2)
		assert.Error(t, err)
	})
}
...
func BenchmarkPlain(b *testing.B) {
	...
	b.Run("Exported", func(b *testing.B) {
		...
		for i := 0; i < b.N; i++ {
			_, err = PlainHash(WrapPassword("password"))
		}
	})
