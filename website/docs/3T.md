...
See below as an example which connects to `localhost` on port 27017. It uses the user `username` with which to authenticate to FerretDB and specifies `password` as the password to use. It then specifies the PLAIN authentication mechanism.
