...
1. Check [documentdb_core.control](https://github.com/FerretDB/documentdb/blob/ferretdb/pg_documentdb_core/documentdb_core.control)
   has the `default_version` field set to the new version to release.
   If not, commit and push updates to a PR and merge it.
2. Check the tag exists for the new version to release.
   Ensure to sync [FerretDB/documentdb](https://github.com/FerretDB/documentdb) fork.
3. Create draft release in [FerretDB/documentdb](https://github.com/FerretDB/documentdb)
   and choose an existing tag with autogenerated release notes.
4. Find [ferretdb_packages CI build](https://github.com/FerretDB/documentdb/actions/workflows/ferretdb_packages.yml?query=event%3Apush)
   for the tag to release.
5. Upload `.deb` packages to the draft release.
6. Publish release on [FerretDB/documentdb](https://github.com/FerretDB/documentdb).
