...
func TestCreateIndexesCommandCompat(t *testing.T) {
	...
	for name, tc := range map[string]struct { //nolint:vet // for readability
		...
		t.Run(name, func(tt *testing.T) {
			...
			targetListRes := integration.FetchAll(t, ctx, targetCursor)
			compatListRes := integration.FetchAll(t, ctx, compatCursor)
...
func TestDropIndexesCommandCompat(tt *testing.T) {
	...
	for name, tc := range map[string]struct { //nolint:vet // for readability
		...
		resultType integration.CompatTestCaseResultType // optional, defaults to NonEmptyResult
		skip       string                               // optional, skip test with a specified reason
	}{
			...
			for i := range targetCollections {
				...
				tt.Run(targetCollection.Name(), func(tt *testing.T) {
					...
					if tc.toCreate != nil {
						...
						targetList := integration.FetchAll(t, ctx, targetCursor)
						compatList := integration.FetchAll(t, ctx, compatCursor)
