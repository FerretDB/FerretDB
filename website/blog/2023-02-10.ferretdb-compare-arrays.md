...
However, when comparing values of different types, the [BSON Comparison Order](#bson-comparison-order) 
is used to determine the result.
In the case of array comparison against other BSON value, 
the comparison may use the BSON comparison order or an element from the array may be 
selected to compare against the other BSON value.
...
<<<<<<< HEAD
When comparing two BSON arrays, each index is iterated in order, and the BSON comparison order
is used to compare the values at each index.
...
2. Compare first index `3` and `3` using comparison order, they are both numbers.
...
an element within the array is used to compare against other BSON types.

[Sorting](#sorting)  used in `.sort({<<field>>: <<order>>})` uses following rules
to compare array and other BSON types.

* For array and other BSON type comparison, ascending sort uses the smallest element of any type from the array.
* For array and other BSON type comparison, descending sort uses the largest element of any type from the array.
* empty array is less than null.

[Filtering](#gt-lt-filter-operator) used in `$lt` and `$gt` operators use following rules
to compare array and other BSON types.
* For array and other BSON type comparison, `$lt` uses the smallest element of the same type as other BSON type from the array.
* For array and other BSON type comparison, `$gt` uses the largest element of the same type as other BSON type from the array.
...
Comparing `2` and `[3, 1]`, the result is different based on
ascending sort or descending sort.

Following steps show using ascending sort to compare `2` and `[3, 1]`.
...
Following steps show using descending sort to compare `2` and `[3, 1]`.
...
Unlike [arrays](#sorting-arrays) which uses the largest or smallest element
from an array, nested array uses BSON comparison order of array in sorting.
compares each index using comparison order
...
Nested arrays `nested-number` and `nested-null`,
