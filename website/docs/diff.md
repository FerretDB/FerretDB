...
5. When insert command is called, insert documents must not have duplicate keys.
6. Database and collection names restrictions:
...
7. For Tigris, FerretDB requires Tigris schema validation for `create` command: validator must be set as `$tigrisSchemaString`.
