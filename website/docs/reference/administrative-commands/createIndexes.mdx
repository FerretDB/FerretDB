...
| `key`                     | document | A single document that contains the field(s) to index and their sort order or type (e.g. `1`, `-1`, `text` for full-text indexes, and `cosmosSearch` for vector indexes). |
