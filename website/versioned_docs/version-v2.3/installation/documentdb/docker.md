...
[`ghcr.io/ferretdb/postgres-documentdb:17-0.104.0-ferretdb-2.3.1`](https://ghcr.io/ferretdb/postgres-documentdb:17-0.104.0-ferretdb-2.3.1)
...
:::tip
We strongly recommend specifying the full image tag (e.g., `17-0.104.0-ferretdb-2.3.1`)
...
1. You can run the image with the following command:
   ...
   docker run -d \
     ...
     -p 5432:5432 \
     ghcr.io/ferretdb/postgres-documentdb:17-0.104.0-ferretdb-2.3.1
   ```
...
2. If you have `psql` installed, you can connect to the PostgreSQL with DocumentDB extension with the following command:
   ...
   ```sh
   docker run --rm -it ghcr.io/ferretdb/postgres-documentdb:17-0.104.0-ferretdb-2.3.1 \
     psql postgres://<username>:<password>@localhost:5432/postgres
   ```
...
You can find the correct tag in the DocumentDB release notes (for example: `17-0.104.0-ferretdb-2.3.1`).
Then run:
