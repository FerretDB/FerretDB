...
**For creating and submitting a pull request, please follow these guides:** 

1. Please create a **draft pull request** when you start working on an issue. 
   Once it’s ready for review, mark it as such.
   You don’t need to switch between draft and non-draft states after that
2. If the pull request is related to some issues, please mention the issue number in the pull request **description** like `Closes #{issue_number}.` or `Closes org/repo#{issue_number}.`. (You can just follow the pull request template)
   If you propose a tiny fix, there is no needed to create a new issue.
3. In the pull request review conversations, please either leave a new comment or resolve (close) the conversation, which ensures the other people can read all comments.
   But **do not** do that simultaneously.
4. Please don't forget to click "re-request review" buttons once PR is ready for re-review.
5. During your development, commit messages (both titles and bodies) are not important and can be "WIP" or anything else.
