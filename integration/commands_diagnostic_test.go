	...
	for _, elem := range a {
		...
		case "system":
			...
			for _, subElem := range elem.Value.(bson.D) {
				switch subElem.Key {
				case "currentTime":
					assert.IsType(t, bson.DateTime(0), subElem.Value)
					elemForComparsion = append(elemForComparsion, bson.E{"currentTime", bson.DateTime(0)})
