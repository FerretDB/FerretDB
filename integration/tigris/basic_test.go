...
import (
	...
	"github.com/FerretDB/FerretDB/integration/tigris/shareddata"
)
...
func TestMostCommandsAreCaseSensitive(t *testing.T) {
	...
	err := res.Err()
	require.Error(t, err)
	integration.AssertEqualError(t, mongo.CommandError{Code: 59, Name: "CommandNotFound", Message: `no such command: 'listcollections'`}, err)
...
func TestInsertFind(t *testing.T) {
	...
	for _, expected := range docs {
		...
		t.Run(fmt.Sprint(id), func(t *testing.T) {
			...
			integration.AssertEqualDocuments(t, expected, actual[0])
		})
	}
}
