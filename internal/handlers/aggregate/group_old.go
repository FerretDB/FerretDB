...
// func ParseOperatorsx(ctx *GroupContext, doc *types.Document) (*string, error) {
...
// 		switch v := value.(type) {
// 		case *types.Document:
// 			parsed, err := ParseOperatorsx(ctx, v)
// 			if err != nil {
// 				return err
// 			}
...
// 		case *types.Document:
// 			parsed, err := ParseOperatorsx(ctx, param)
// 			if err != nil {
// 				return err
// 			}
...
// func ParseOperators(doc *types.Document) ([]Field, error) {
// 	for _, key := range doc.Keys() {
// 		value := must.NotFail(doc.Get(key))
// 		switch key {
// 		case "$add", "$subtract", "$multiply", "$divide":
