...
func testCollection(t *testing.T, ctx context.Context, r *Registry, db *pgxpool.Pool, dbName, collectionName string) {
	...
	created, err := r.CollectionCreate(ctx, CreateCollectionParams{dbName, collectionName, 0, 0})
	require.NoError(t, err)
...
func TestRenameCollection(t *testing.T) {
	...
	created, err := r.CollectionCreate(ctx, CreateCollectionParams{dbName, oldCollectionName, 0, 0})
	require.NoError(t, err)
	require.True(t, created)
