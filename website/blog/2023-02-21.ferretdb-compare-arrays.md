...
All number types are the same BSON comparison order, regardless of
`double`, `int32`, `int64` or `decimal`.

### Comparison Rules of Filter

When comparing an array filter argument with array collections,
each index is iterated in order.
...
For comparing non-array filter argument with array in the collection, `$lt` filter uses
