...
import CreateIndex from '!!raw-loader!./create-indexes/1-create-simple-index.request.js';
import CreateUniqueIndex from '!!raw-loader!./create-indexes/2-create-unique-index.request.js';
import CreateCompoundIndex from '!!raw-loader!./create-indexes/3-create-compound-index.request.js';
import CreatePartialIndex from '!!raw-loader!./create-indexes/4-create-partial-index.request.js';
import CreateSparseIndex from '!!raw-loader!./create-indexes/5-create-sparse-index.request.js';
...
db.runCommand({
  ...
  indexes: [
    {
      key: {
        '<field1>': <index_type>, // 1 for ascending, -1 for descending
        '<field2>': <index_type> // optional
      },
      name: '<index_name>',
...
<!-- majorly fields such as arguments, modifiers, stage, operators, parameters, options supported by the command in a table (if necessary or available). Takes in the following parameters: -->
...
| `key`                     | document | A single document that contains the field(s) to index and their sort order or index type (e.g. `1`, `-1`, `text` for full-text indexes, and `cosmosSearch` for vector indexes). |
