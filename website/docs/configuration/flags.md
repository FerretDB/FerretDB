...
| `--state-dir`  | Path to the FerretDB state directory<br />(set to a blank string to disable) | `FERRETDB_STATE_DIR` | `.`<br />(`/state` for Docker) |
...
| `--debug-addr`           | Listen address for HTTP handlers for metrics, pprof, etc<br />(set to a blank string to disable) | `FERRETDB_DEBUG_ADDR`           | `127.0.0.1:8088`<br />(`:8088` for Docker)   |
