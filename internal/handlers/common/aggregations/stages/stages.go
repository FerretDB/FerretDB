...
func NewProducerStage(stage *types.Document, db, collection string, previousStages []string, datasource DataSource) (aggregations.ProducerStage, error) { //nolint:lll // for readability
	if stage.Len() != 1 {
		return nil, commonerrors.NewCommandErrorMsgWithArgument(
			commonerrors.ErrStageInvalid,
			"A pipeline stage specification object must contain exactly one field.",
