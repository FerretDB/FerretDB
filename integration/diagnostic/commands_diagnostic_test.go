...
func TestCommandsDiagnosticExplain(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		...
		t.Run(name, func(tt *testing.T) {
			...
			serverInfo := integration.ConvertDocument(t, explainResult["serverInfo"].(bson.D))
...
func TestCommandsDiagnosticGetLog(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		...
		t.Run(name, func(tt *testing.T) {
			...
			m := res.Map()
			k := integration.CollectKeys(t, res)
...
func TestCommandsDiagnosticHostInfo(t *testing.T) {
	...
	assert.Equal(t, []string{"system", "os", "extra", "ok"}, integration.CollectKeys(t, actual))

	os := m["os"].(bson.D)
	assert.Equal(t, []string{"type", "name", "version"}, integration.CollectKeys(t, os))

	system := m["system"].(bson.D)
	keys := integration.CollectKeys(t, system)
...
func TestCommandsDiagnosticListCommands(t *testing.T) {
	...
	assert.Equal(t, float64(1), m["ok"])
	assert.Equal(t, []string{"commands", "ok"}, integration.CollectKeys(t, actual))
...
func TestCommandsDiagnosticValidate(t *testing.T) {
	...
	actual := integration.ConvertDocument(t, doc)
...
func TestCommandsDiagnosticWhatsMyURI(t *testing.T) {
	...
	for _, collection := range []*mongo.Collection{collection1, collection2} {
		...
		doc := integration.ConvertDocument(t, actual)
		keys := doc.Keys()
		values := doc.Values()
