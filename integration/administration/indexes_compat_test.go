...
func TestListIndexesCompat(t *testing.T) {
	...
	for i := range targetCollections {
		...
		t.Run(targetCollection.Name(), func(t *testing.T) {
			...
			targetRes := integration.FetchAll(t, ctx, targetCur)
			compatRes := integration.FetchAll(t, ctx, compatCur)
		...
		t.Run(name, func(t *testing.T) {
			...
			for i := range targetCollections {
				...
				t.Run(targetCollection.Name(), func(t *testing.T) {
					...
					targetIndexes := integration.FetchAll(t, ctx, targetCur)
					compatIndexes := integration.FetchAll(t, ctx, compatCur)
...
func TestCreateIndexesCommandCompat(t *testing.T) {
	...
	for name, tc := range map[string]struct { //nolint:vet // for readability
		...
		t.Run(name, func(t *testing.T) {
			...
			if tc.resultType == integration.EmptyResult {
...
func TestDropIndexesCommandCompat(t *testing.T) {
	...
	for name, tc := range map[string]struct { //nolint:vet // for readability
		...
		resultType integration.CompatTestCaseResultType // optional, defaults to integration.NonEmptyResult
		skip       string                               // optional, skip test with a specified reason
	}{
			...
			for i := range targetCollections {
				...
				t.Run(targetCollection.Name(), func(t *testing.T) {
					...
					if tc.toCreate != nil {
						...
						targetList := integration.FetchAll(t, ctx, targetCur)
						compatList := integration.FetchAll(t, ctx, compatCur)
