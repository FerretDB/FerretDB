...
func TestCommandsAdministrationCreateDropList(t *testing.T) {
	...
	integration.AssertEqualError(t, expectedErr, err)
		...
		t.Run(name, func(t *testing.T) {
			...
			for k, item := range tc.expected {
				...
				if it, ok := item.(primitive.D); ok {
					z := m[k].(primitive.D)
					integration.AssertEqualDocuments(t, it, z)
				} else {
...
func TestCommandsAdministrationServerStatusMetrics(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			actualMetric, err := integration.ConvertDocument(t, actual).GetByPath(tc.metricsPath)
			assert.NoError(t, err)
