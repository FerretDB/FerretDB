...
_This is a duplicate post from [Tutornix](https://tutornix.com/how-to-install-ferretdb-mongodb-alternative-on-ubuntu-22-0420-04/) about installing FerretDB on Ubuntu; we are grateful to the author for the permission to repost this on our blog._
...
- **`--handler`=`pg`** : Backend handler: `dummy`, `pg` (Default), `tigris` \
  (`$FERRETDB_HANDLER`).
