...
func getSortType(key string, value any) (types.SortType, error) {
	...
	if err != nil {
		...
		case errUnexpectedType:
			...
			return 0, NewCommandErrorMsgWithArgument(ErrSortBadValue, fmt.Sprintf(`Illegal key in $sort specification: %v: %v`, key, value), "$sort")
	...
	default:
		return 0, NewCommandErrorMsgWithArgument(ErrSortBadOrder, "$sort key ordering must be 1 (for ascending) or -1 (for descending)", "$sort")
	}
}
