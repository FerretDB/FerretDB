...
func init() {
	...
	// 1. When we work from the FerretDB repo (building binaries, running integration tests, etc),
	...
	if buildVersion != "" {
		if info.Version != unknown {
			panic(fmt.Sprintf("buildVersion=%q, info.Version=%q", buildVersion, info.Version))
		}
	...
	if buildCommit != "" && info.Commit != buildCommit {
